  ...
  stages {
    ...
    stage('Submit Stack') {
      steps {
        withAWS(roleAccount:'327173749814', role:'cloudformation') {
	  script{
		string slackname = sh(script: """ aws cloudformation list-stacks --query 'StackSummaries[*]['StackName']' --stack-status-filter UPDATE_ROLLBACK_COMPLETE CREATE_COMPLETE UPDATE_COMPLETE --region 'ap-southeast-1' --output text""",returnStdout: true).trim() | grep 'vpctestforaven'
		echo slackname 
