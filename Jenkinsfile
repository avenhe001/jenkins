pipeline {
  ...
  stages {
    ...
    stage('Submit Stack') {
      steps {
        withAWS(roleAccount:'327173749814', role:'cloudformation') {
		string count = sh(script: """ aws cloudformation list-stacks --stack-status-filter CREATE_COMPLETE --region 'ap-southeast-1' --query '[*].StackName' --output text""",returnStdout: true).trim()
		echo count
