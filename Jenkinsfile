  ...
  stages {
    ...
    stage('Submit Stack') {
      steps {
        withAWS(roleAccount:'327173749814', role:'cloudformation') {
	  script{
		string slackname = sh(script: """ aws cloudformation list-stacks --query 'StackSummaries[*]['StackName']' --stack-status-filter CREATE_COMPLETE UPDATE_COMPLETE --region 'ap-southeast-1' --output text""",returnStdout: true).trim()
		echo slackname
