  ...
  stages {
    ...
    stage('Submit Stack') {
	  ...
	  script{
		...
		echo slackname 
		result = sh(script: """ aws cloudformation list-stacks --query 'StackSummaries[*]['StackName']' --stack-status-filter UPDATE_ROLLBACK_COMPLETE CREATE_COMPLETE UPDATE_COMPLETE --region 'ap-southeast-1' --output text | grep 'vpctestforaven' """)
		echo result
